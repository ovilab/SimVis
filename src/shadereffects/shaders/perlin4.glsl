...
highp float cnoise(highp vec4 P)
{
  ...
  Pi0 = mod289(Pi0);
  Pi1 = mod289(Pi1);
  highp vec4 Pf0 = fract(P); // Fractional part for interpolation
  highp vec4 Pf1 = Pf0 - 1.0; // Fractional part - 1.0
  highp vec4 ix = vec4(Pi0.x, Pi1.x, Pi0.x, Pi1.x);
  highp vec4 iy = vec4(Pi0.yy, Pi1.yy);
  highp vec4 iz0 = vec4(Pi0.zzzz);
  highp vec4 iz1 = vec4(Pi1.zzzz);
  highp vec4 iw0 = vec4(Pi0.wwww);
  highp vec4 iw1 = vec4(Pi1.wwww);

  highp vec4 ixy = permute(permute(ix) + iy);
  highp vec4 ixy0 = permute(ixy + iz0);
  highp vec4 ixy1 = permute(ixy + iz1);
  highp vec4 ixy00 = permute(ixy0 + iw0);
  highp vec4 ixy01 = permute(ixy0 + iw1);
  highp vec4 ixy10 = permute(ixy1 + iw0);
  highp vec4 ixy11 = permute(ixy1 + iw1);

  highp vec4 gx00 = ixy00 * (1.0 / 7.0);
  highp vec4 gy00 = floor(gx00) * (1.0 / 7.0);
  highp vec4 gz00 = floor(gy00) * (1.0 / 6.0);
  gx00 = fract(gx00) - 0.5;
  gy00 = fract(gy00) - 0.5;
  gz00 = fract(gz00) - 0.5;
  highp vec4 gw00 = vec4(0.75) - abs(gx00) - abs(gy00) - abs(gz00);
  highp vec4 sw00 = step(gw00, vec4(0.0));
  gx00 -= sw00 * (step(0.0, gx00) - 0.5);
  gy00 -= sw00 * (step(0.0, gy00) - 0.5);

  highp vec4 gx01 = ixy01 * (1.0 / 7.0);
  highp vec4 gy01 = floor(gx01) * (1.0 / 7.0);
  highp vec4 gz01 = floor(gy01) * (1.0 / 6.0);
  gx01 = fract(gx01) - 0.5;
  gy01 = fract(gy01) - 0.5;
  gz01 = fract(gz01) - 0.5;
  highp vec4 gw01 = vec4(0.75) - abs(gx01) - abs(gy01) - abs(gz01);
  highp vec4 sw01 = step(gw01, vec4(0.0));
  gx01 -= sw01 * (step(0.0, gx01) - 0.5);
  gy01 -= sw01 * (step(0.0, gy01) - 0.5);

  highp vec4 gx10 = ixy10 * (1.0 / 7.0);
  highp vec4 gy10 = floor(gx10) * (1.0 / 7.0);
  highp vec4 gz10 = floor(gy10) * (1.0 / 6.0);
  gx10 = fract(gx10) - 0.5;
  gy10 = fract(gy10) - 0.5;
  gz10 = fract(gz10) - 0.5;
  highp vec4 gw10 = vec4(0.75) - abs(gx10) - abs(gy10) - abs(gz10);
  highp vec4 sw10 = step(gw10, vec4(0.0));
  gx10 -= sw10 * (step(0.0, gx10) - 0.5);
  gy10 -= sw10 * (step(0.0, gy10) - 0.5);

  highp vec4 gx11 = ixy11 * (1.0 / 7.0);
  highp vec4 gy11 = floor(gx11) * (1.0 / 7.0);
  highp vec4 gz11 = floor(gy11) * (1.0 / 6.0);
  gx11 = fract(gx11) - 0.5;
  gy11 = fract(gy11) - 0.5;
  gz11 = fract(gz11) - 0.5;
  highp vec4 gw11 = vec4(0.75) - abs(gx11) - abs(gy11) - abs(gz11);
  highp vec4 sw11 = step(gw11, vec4(0.0));
  gx11 -= sw11 * (step(0.0, gx11) - 0.5);
  gy11 -= sw11 * (step(0.0, gy11) - 0.5);

  highp vec4 g0000 = vec4(gx00.x,gy00.x,gz00.x,gw00.x);
  highp vec4 g1000 = vec4(gx00.y,gy00.y,gz00.y,gw00.y);
  highp vec4 g0100 = vec4(gx00.z,gy00.z,gz00.z,gw00.z);
  highp vec4 g1100 = vec4(gx00.w,gy00.w,gz00.w,gw00.w);
  highp vec4 g0010 = vec4(gx10.x,gy10.x,gz10.x,gw10.x);
  highp vec4 g1010 = vec4(gx10.y,gy10.y,gz10.y,gw10.y);
  highp vec4 g0110 = vec4(gx10.z,gy10.z,gz10.z,gw10.z);
  highp vec4 g1110 = vec4(gx10.w,gy10.w,gz10.w,gw10.w);
  highp vec4 g0001 = vec4(gx01.x,gy01.x,gz01.x,gw01.x);
  highp vec4 g1001 = vec4(gx01.y,gy01.y,gz01.y,gw01.y);
  highp vec4 g0101 = vec4(gx01.z,gy01.z,gz01.z,gw01.z);
  highp vec4 g1101 = vec4(gx01.w,gy01.w,gz01.w,gw01.w);
  highp vec4 g0011 = vec4(gx11.x,gy11.x,gz11.x,gw11.x);
  highp vec4 g1011 = vec4(gx11.y,gy11.y,gz11.y,gw11.y);
