//const highp vec3 noiseVectors[128] = vec3[](
...
const highp vec3 noiseVectors[64] = vec3[](
...
highp float ambientOcclusion(highp sampler2D depthTexture, highp vec3 position, highp vec3 normal,
...
//    highp vec3 tangent = normalize(makePerpendicular(normal));
//    highp vec3 binormal = normalize(cross(normal, tangent));

//    highp mat3 basis = mat3(binormal, tangent, normal);

    highp float occlusion = 0.0;
    for(int i = 0; i < samples; i++) {
//        highp vec3 samplePosition = position.xyz + radius * basis * noiseVectors[i];
        highp vec3 samplePosition = position.xyz + radius * noiseVectors[i];

        highp vec2 texCoord = texCoordFromPosition(samplePosition, viewMatrix, projectionMatrix);
        highp float sampleDepth = texture(depthTexture, texCoord).r;
