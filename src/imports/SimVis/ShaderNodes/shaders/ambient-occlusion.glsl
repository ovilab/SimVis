const vec3 noiseVectors[128] = vec3[](
...
float ambientOcclusion(sampler2D depthTexture, vec3 position, vec3 normal,
                       int samples, float radius,
                       mat4 viewMatrix, mat4 projectionMatrix) {
    vec2 positionTexCoord = texCoordFromPosition(position, viewMatrix, projectionMatrix);
    float fragDepth = texture(depthTexture, positionTexCoord).r;

    vec3 tangent = normalize(makePerpendicular(normal));
    vec3 binormal = normalize(cross(normal, tangent));

    mat3 basis = mat3(binormal, tangent, normal);

    float occlusion = 0.0;
    for(int i = 0; i < samples; i++) {
        vec3 samplePosition = position.xyz + radius * basis * noiseVectors[i];
