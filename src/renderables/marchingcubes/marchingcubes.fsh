...
void main() {
    lowp float n = surface4(vec4(scaledPosition.xyz, 0.1*time)); // From perlin3.fsh
...
//    lowp float n3 = surface4(0.2*vec4(scaledPosition.yzx,time*0.0)); // From perlin4.fsh
    ...
    lowp float lightValue = 0.2 + light*clamp(dot(normalize(lightPosition), normal), 0.0, 1.0);
